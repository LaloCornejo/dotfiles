...
ls: {
use_ls_colors: false # Disabled for performance
                clickable_links: false # Disabled for performance
              }
...
history: {
max_size: 10_000 # Reduced for performance
           sync_on_enter: false # Disabled for performance - history written on exit
...
case_sensitive: false 
                 quick: true    
                 partial: true    
                 algorithm: "prefix"    # Changed from fuzzy to prefix for speed
                 external: {
enable: true # Re-enabled for carapace completions
         max_results: 50 # Increased for carapace
...
shell_integration: {
# Minimal shell integration for performance
osc2: false  # Disabled for performance
       osc7: true   # Keep for terminal directory sync
       osc8: false  # Disabled for performance
       osc9_9: false
       osc133: false # Disabled for performance
       osc633: false # Disabled for performance
       reset_application_mode: false # Disabled for performance
               }
...
plugin_gc: {
# Optimized plugin garbage collection
                     default: {
enabled: true 
          stop_after: 5sec # Reduced from 10sec for faster cleanup
                                              }
plugins: {}
                     }
...
# Optimized PATH configuration - only add if not already present
let custom_paths = [
    '~/.cargo/bin'
    '~/go/bin'
    '/usr/local/go/bin'
...
$env.PATH = ($env.PATH | split row (char esep) | append $custom_paths | uniq | str join (char esep))
...
# Carapace completion setup and optimizations
$env.CARAPACE_BRIDGES = 'zsh,fish,bash,inshellisense' # Enable cross-shell completions
$env.CARAPACE_LOG_LEVEL = 'error' # Reduce logging for performance

# Removed duplicate PATH assignment for performance
